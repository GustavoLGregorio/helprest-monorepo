...
// 3. Helper to check if a command exists in PATH
...
try {
    // Create temporary npm shim if npm is missing from system PATH
    if (!hasCommand("npm")) {
...
`;
    ...
    // Clean up temporary bin folder after compilation completes or fails
    cleanupBinDir();
}
